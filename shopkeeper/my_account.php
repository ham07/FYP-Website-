<?php
session_start();
include "../UserPanel/includes/header3.php";
 ...
 <div class="container">
   ...
   if(isset($_POST['submit1']))
   {
     ...
     else
     {
       ...
       $query="select * from login where id='$_SESSION[shopkeeperID]'";
       $result=$connection->query($query);
        while($row=mysqli_fetch_array($result))
        {
          if($hashed == $row['password'])
         {
           $query1="update login set username='$name',password='$hashed1' where id=$_SESSION[shopkeeperID]";
  ...
  <h5 class="text-danger text-left"><?=$msg;?></h5>
   <button class="btn btn-primary btn-lg "style="margin-top:20px;" type="submit"name="submit1">
    ...
    <input style="margin-top:20px;" type="button"class="btn btn-primary btn-lg " onclick="window.location.href = 'Shopkeeperhome.php';" value="Back"/>
